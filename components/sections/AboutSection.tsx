...
export default function AboutSection() {
  ...
  return (
    <section
      id="about"
      ref={ref}
      className="grid grid-cols-1 md:grid-cols-2 gap-[60px] md:gap-[100px] items-start py-[160px] px-[5vw] max-w-[1300px] mx-auto"
    >
      {/* Left */}
      <div>
        <motion.div
          ...
          className="font-mono text-[10px] tracking-[.22em] uppercase text-[var(--accent)] mb-5"
        >
          // About
        </motion.div>
        <motion.h2
      ...
      <div className="pt-5">
        ...
        >
          <strong className="text-[var(--cream)] font-medium">Fauzan Nurhikmah</strong> — Senior Engineer & Full Stack Developer.
          I specialise in the full spectrum: from WebGL shaders and React component libraries to distributed backend systems processing millions of events.
        </motion.p>
        <motion.p
          variants={fadeUp}
          initial="hidden"
        ...
        >
          {stats.map((s) => (
            ...
            >
              <div className="font-display text-[52px] leading-none text-[var(--cream)] tracking-[.02em]">
                {s.val}
                <span className="text-[28px] text-[var(--accent)]">{s.sup}</span>
              </div>
              <div className="font-mono text-[10px] tracking-[.16em] uppercase text-[var(--muted)] mt-2">
                {s.key}
              </div>
            </div>
          ))}
        ...
        >
          {strengths.map((s) => (
            <div
              key={s.title}
              className="strength-row flex items-center gap-4 py-3.5 transition-all duration-300 group"
              style={{ borderBottom: '1px solid var(--border)' }}
            >
              <div
                className="w-8 h-8 flex items-center justify-center text-sm flex-shrink-0 border border-[var(--border)] transition-all duration-300 group-hover:border-[rgba(232,255,71,.4)] group-hover:bg-[rgba(232,255,71,.06)]"
              >
                {s.icon}
              </div>
              <div>
