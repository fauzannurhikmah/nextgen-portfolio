...
const projects = [
  ...
  },
  {
    ...
    tags: ['Next.js', 'Rust WASM', 'WebSockets'],
  },
  {
    num: '03',
  ...
  },
  {
    ...
    tags: ['Rust', 'Docker SDK', 'Open Source'],
  },
]
        ...
        <div className="mb-20">
          ...
          >
            ...
            <em
              style={{
                fontStyle: 'normal',
                WebkitTextStroke: '1px rgba(240,236,227,.25)',
                color: 'transparent',
              }}
            >
              Projects
        ...
        <div className="flex flex-col gap-px">
          {projects.map((proj, i) => (
            <motion.div
              ...
              className="proj-item grid gap-8 items-center py-9 group cursor-none transition-all duration-[350ms]"
              style={{
            ...
            >
              <div className="font-mono text-[12px] tracking-[.1em] text-[var(--muted)]">
                {proj.num}
              </div>
              <div>
                <h3
                  className="font-semibold text-[var(--cream)] mb-2.5 transition-colors duration-200 group-hover:text-[var(--accent)]"
                  style={{ fontSize: 'clamp(20px, 2.5vw, 30px)' }}
                >
                  {proj.title}
                </h3>
                <p className="text-sm leading-[1.6] text-[var(--muted)] max-w-[600px]">
                  {proj.desc}
                </p>
              </div>
              <div className="hidden md:flex flex-col items-end gap-3.5">
                <div className="flex gap-2 flex-wrap justify-end">
                  {proj.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-3 py-1 border border-[var(--border)] font-mono text-[9px] tracking-[.1em] uppercase text-[var(--muted)]"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <div
                  className="w-[42px] h-[42px] flex items-center justify-center text-lg text-[var(--muted)] border border-[var(--border)] -rotate-45 transition-all duration-300 group-hover:border-[var(--accent)] group-hover:text-[var(--accent)] group-hover:rotate-0 group-hover:bg-[rgba(232,255,71,.06)]"
                >
                  →
                </div>
              </div>
