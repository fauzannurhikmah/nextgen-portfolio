...
export default function ContactSection() {
  ...
  return (
    <section id="contact" className="py-[160px] px-[5vw] text-center relative overflow-hidden" ref={ref}>
      ...
      <div className="max-w-[800px] mx-auto relative z-[1]">
        ...
        >Let's build together</motion.div>

        <motion.div
          initial={{ opacity:0, y:80 }} animate={inView ? { opacity:1, y:0 } : {}}
          transition={{ duration:1.4, delay:0.1, ease:[0.23,1,0.32,1] }}
          className="font-display leading-[.88] tracking-[.01em] mb-10"
          style={{ fontSize:'clamp(64px,11vw,160px)' }}
        >LET'S<br />TALK.</motion.div>
        ...
        <motion.a href="mailto:fauzan@example.com"
          initial={{ opacity:0, y:50 }} animate={inView ? { opacity:1, y:0 } : {}}
          transition={{ duration:0.9, delay:0.3, ease:[0.23,1,0.32,1] }}
          className="inline-block font-display tracking-[.04em] text-[var(--cream)] no-underline pb-2 transition-colors duration-300 hover:text-[var(--accent)]"
          style={{ fontSize:'clamp(28px,4.5vw,56px)', borderBottom:'1px solid rgba(232,255,71,.3)' }}
        >fauzan@example.com</motion.a>
        ...
        <div className="mt-[100px] pt-8 flex justify-between items-center"
          style={{ borderTop:'1px solid var(--border)' }}>
          <span className="font-mono text-[10px] tracking-[.12em] text-[rgba(240,236,227,.2)] uppercase">© 2025 Fauzan Nurhikmah</span>
          <span className="font-mono text-[10px] tracking-[.12em] text-[rgba(240,236,227,.2)] uppercase">Built with craft, not templates</span>
        </div>
      </div>
    </section>
  )
