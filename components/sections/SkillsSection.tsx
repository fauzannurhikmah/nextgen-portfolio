...
const skills = [
  ...
  { cat: 'Data',      name: 'PostgreSQL / Redis',  tags: ['ClickHouse','Kafka','Prisma'],                   bar: 0.80 },
  { cat: 'Infra',     name: 'AWS / Kubernetes',    tags: ['Terraform','Docker','CI/CD'],                    bar: 0.82 },
  { cat: 'Motion',    name: 'GSAP / Framer',       tags: ['ScrollTrigger','Physics','SVG'],                 bar: 0.90 },
]
    ...
    <section id="skills" className="py-[160px] px-[5vw]">
      <div className="max-w-[1300px] mx-auto" ref={ref}>
        <div className="flex items-end justify-between flex-wrap gap-10 mb-20">
          ...
          >
            The <em style={{ fontStyle:'normal', color:'var(--accent)' }}>Stack</em>
          </motion.h2>
          <motion.p
            initial={{ opacity:0, y:50 }} animate={inView ? { opacity:1, y:0 } : {}}
        ...
        <table className="w-full border-collapse">
          <tbody>
            {skills.map((skill, i) => <SkillRow key={skill.name} skill={skill} index={i} />)}
          </tbody>
        </table>
      </div>
    </section>
...
function SkillRow({ skill, index }: { skill: typeof skills[0]; index: number }) {
  ...
  useEffect(() => {
    if (inView && barRef.current) {
      ...
      setTimeout(() => { el.style.transform = `scaleX(${skill.bar})` }, 300 + index * 80)
    }
  ...
  return (
    <motion.tr
      ref={rowRef}
      initial={{ x:-60, opacity:0 }} animate={inView ? { x:0, opacity:1 } : {}}
      transition={{ duration:0.8, delay:index*0.08, ease:[0.23,1,0.32,1] }}
      className="group cursor-none"
      style={{ borderBottom:'1px solid var(--border)' }}
    >
      <td className="py-5 align-middle font-mono text-[10px] tracking-[.2em] uppercase text-[var(--muted)] w-40">{skill.cat}</td>
      <td className="py-5 align-middle text-[18px] font-semibold text-[var(--cream)] transition-colors duration-200 group-hover:text-[var(--accent)]">{skill.name}</td>
      <td className="py-5 align-middle px-5">
        <div className="flex gap-2 flex-wrap">
          {skill.tags.map(tag => (
            <span key={tag} className="px-3 py-1 border border-[var(--border)] font-mono text-[10px] tracking-[.06em] text-[var(--muted)] transition-all duration-200 group-hover:border-[rgba(232,255,71,.25)] group-hover:text-[rgba(232,255,71,.7)]">
              {tag}
            </span>
          ))}
        </div>
